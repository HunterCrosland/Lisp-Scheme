...
(defun node2var (n c k)
  ; calculate color variable with given formula
  (+ c (* k (- n 1)))
) ; defun
...
(defun at-least-one-color (n c k)
  (if (>= c k) nil ; finished recursion (or invalid input)
  ; create cnf list of possible colors for n
  (append (list (node2var n c k)) (at-least-one-color n (+ c 1) k)))
) ; defun

  ; HELPER function for at-most-one-color below
  (defun c-squared-loop (n c0 k c)
    (if (>= c k) nil ; finished recursion (or invalid input)
    ; create cnf list to specify impossible color combinations 
    (append (list (list (- (node2var n c0 k)) (-(node2var n c k)))) (c-squared-loop n c0 k (+ c 1))))
  ) ; defun
  ...
  (defun at-most-one-color (n c k)
    (if (>= c k) nil 
    (append (c-squared-loop n c k (+ c 1)) (at-most-one-color n (+ c 1) k)))
  ) ; defun
...
(defun generate-node-clauses (n k)
  ; applying at least one and at most one together yields exactly one
...
(defun c-loop (e c k)
  ...
  (if (>= c k) (cons (list (- (node2var x c k)) (- (node2var y c k))) nil)
               (cons (list (- (node2var x c k)) (- (node2var y c k))) (c-loop e (+ c 1) k))
